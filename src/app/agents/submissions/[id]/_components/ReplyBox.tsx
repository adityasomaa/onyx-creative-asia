...
import { sendWaReplyAction } from "../../actions";

/**
 * Reply composer.
 *
 * - For source='whatsapp' + wa_kind='contact': real-time WA send through
 *   Fonnte, gated by wa-safety, mirrored into submission_messages.
 * - For source='whatsapp' + wa_kind='group': not supported (banner instead).
 * - For source='form' / 'email': UI is hidden; operator replies via
 *   email externally (or future channel-aware compose).
 */
export default function ReplyBox({
  ...
  function trySend() {
    setErr(null);
    if (!draft.trim()) return setErr("Message is empty.");
    setStage("confirm");
  }
  ...
  function actuallySend() {
    ...
    startTransition(async () => {
      const res = await sendWaReplyAction(submissionId, draft);
      if (!res.ok) {
      ...
      <div className="flex items-center justify-between mt-2 gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          {stage === "compose" ? (
            ...
            <>
              <span className="text-[11px] italic opacity-65">Send now?</span>
              <button
                type="button"
